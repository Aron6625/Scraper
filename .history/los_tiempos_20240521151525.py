...
def extract_news(uri):
    ...
    main = soup.select_one(f'.content-column')

    region = main.find('div', class_='region')
    content_article = region.find('div', class_='clearfix')
    ...
    author_element= article.find('div', class_='autor').select_one(f'.field-item a')
    ...
    print("pagina escrapeada correctamente!")
    news.append({
        'title': title,
        'description':description,
        'image_url': link,
